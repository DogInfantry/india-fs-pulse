...
from common import record_source, write_processed  # noqa: E402
...
def main() -> None:
    ...
    df = generate()
    write_processed(df, "survey_SYNTHETIC_nps")
    record_source("survey_SYNTHETIC_nps",
                  url="https://github.com/DogInfantry/india-fs-pulse/blob/main/analysis/04_survey_nps.py",
                  publisher="Generated by this repository (SYNTHETIC)",
                  coverage=f"{N:,} respondents x {len(EPISODES)} episodes, seed {SEED}",
                  rows=len(df), licence="Apache-2.0",
                  note="SYNTHETIC. Not a real survey. Generated from a documented seeded "
                       "model to demonstrate episode-level NPS, because published episode "
                       "benchmarks are proprietary.")
